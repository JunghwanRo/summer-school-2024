...
class RRT:
    ...
    def generatePath(self, g_start, g_end, path_planner, rrtstar=False, straighten=False):
        ...
        print(
            "[INFO] {:s}: Searching for path from [{:.2f}, {:.2f}, {:.2f}] to [{:.2f}, {:.2f}, {:.2f}].".format(
                "RRT*" if rrtstar else "RRT", g_start[0], g_start[1], g_start[2], g_end[0], g_end[1], g_end[2]
            )
        )
    ...
    def getRandomPointGaussian(self, sigma_offset=0.0):
        ...
        # Adjust sigma by the inflation offset
        sigma += sigma_offset
    ...
    def buildTree(self, branch_size, rrtstar_neighborhood=None):
        ...
        while not self.tree.valid:
            ...
            if time.time() - start_time > self.timeout:
                print(
                    "[ERROR] {:s}: Timeout limit in buildTree() exceeded ({:.1f} s > {:.1f} s). Ending.".format(
                        "RRT*" if rrtstar else "RRT", time.time() - start_time, self.timeout
                    )
                )
                return
