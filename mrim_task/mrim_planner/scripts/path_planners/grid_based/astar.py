...
class AStar:
    ...
    def halveAndTest(self, path):
        ...
        return [pt1, pt2]
        """
        # Check if there is a direct path between pt1 and pt2
        if not self.grid.obstacleBetween(pt1, pt2):
            return [pt1, pt2]

        # If there's an obstacle, split the path into two halves and check them recursively
        mid_idx = len(path) // 2
        seg1 = self.halveAndTest(path[: mid_idx + 1])
        seg2 = self.halveAndTest(path[mid_idx:])

        # Combine the two segments
        combined_path = seg1[:-1] + seg2  # Avoid duplicating the midpoint
    ...
    def generatePath(self, m_start, m_goal):
        ...
        print(
            "[INFO] A*: Searching for path from [{:.2f}, {:.2f}, {:.2f}] to [{:.2f}, {:.2f}, {:.2f}].".format(
                m_start[0], m_start[1], m_start[2], m_goal[0], m_goal[1], m_goal[2]
            )
        )
    ...
    def searchPath(self, start, goal):
        ...
        while True:
            ...
            if time.time() - start_time > self.timeout:
                print(
                    "[ERROR] A*: Timeout limit in searchPath() exceeded ({:.1f} s > {:.1f} s). Ending.".format(time.time() - start_time, self.timeout)
                )
                return None
    ...
    def neighbors(self, pos):
        ...
        for n in self.neighborhood:
            ...
            if (
                0 <= idx[0] < self.grid.dim[0]
                and 0 <= idx[1] < self.grid.dim[1]
                and 0 <= idx[2] < self.grid.dim[2]
                and not self.grid.idxIsOccupied(idx)
            ):
                neigh.append(idx)
